...
class SRFLP:
    ...
    def calcular(self, solucion_parcial):
        ''''
            Calcula la formula presentada en el enunciado del problema.
            Esta formula representa el esfuerzo medido como la suma de la distancia que los clientes, o quienes preparan los pedidos,
            deben caminar de un puesto a otro
        '''
        total = 0
        for i in range(0, len(solucion_parcial)-1):
            total_medio = 0
            for j in range(i+1, len(solucion_parcial)):
                total_medio = self.arreglo_tamanos[solucion_parcial[i]]/2 + self.arreglo_tamanos[solucion_parcial[j]]/2
